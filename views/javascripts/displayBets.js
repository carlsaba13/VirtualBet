        ...
        function loadBets() {
            ...
            .then(data => {
                for (i in data) {
                    ...
                    console.log(team1);
                    console.log(team2);
                    Odd.findOne({gameID: id}, function (err, odds) {
                        if (err) {
                            console.log("Error occured looking for bet");
                            res.status(500).send();
                        }
                        else if (!odds) { // odd doesn't exist, return 404 error
                            res.status(404).send();
                        } else { // odd exists, move to next function
                            let homeOdds = odds["home"];
                            let awayOdds = odds["away"];
                            console.log(homeOdds, ' and ', awayOdds);
                        }
                    })
                }
